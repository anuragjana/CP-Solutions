...
ll npr(ll n,ll r)
{
    ...
    ll ans = mod_div(factorial[n],factorial[n-r],mod);
    return ans;
}
...
void solve()
{
    ...
    {
        ll sec_cnt = mp[arr[n-2]];
        ll ans = factorial[n];
        ll notans = 0;
        for(ll i=sec_cnt;i<n;i++)
        {
            if(i>=sec_cnt){
                ll rem = n-sec_cnt-1;
                notans = ((notans%mod) + ((npr(i,sec_cnt)%mod)*(factorial[rem]%mod))%mod)%mod;
 
            }
        }
 
        ans = (ans-notans+mod)%mod;
