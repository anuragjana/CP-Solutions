...
void solve(){
    ...
    ll singlecount = 0,doublecount = 0;
    for(auto it : mp)
    {
        debug(it.second);
        singlecount += it.second%2;
        doublecount += it.second/2;
    }
    ...
    vll arr(k,0);
    ll remaining = doublecount%k;
    debug(remaining);
    f(i,0,k-1)
    {
        arr[i]+= 2*(doublecount/k);
    }
 
 
 
    ll answer = arr[0];
    if(2*remaining+singlecount >=k)
        answer++;
