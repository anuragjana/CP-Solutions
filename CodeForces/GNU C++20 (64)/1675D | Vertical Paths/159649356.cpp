...
void dfs(vector<bool> & vis,map<ll,vector<ll>> & mp,vector<vector<int>> & ans,vector<int> &a,ll it )
{
    vis[it]=true;
    vll arr = mp[it];
    a.pb(it);
    if(arr.size()==0)
    {
        ...
        dfs(vis,mp,ans,a,arr[0]);
 
    for(int i=1;i<arr.size();i++)
    {
        vector<int> x;
        dfs(vis,mp,ans,x,arr[i]);
    }
}
