...
ll dfs(ll node,ll parent,ll lr,vector<vector<ll>> &dp)
{
 if(dp[node][lr]!=-1)
  return dp[node][lr];
 ll sum = 0;
 for(auto child : adj[node])
 {
  ...
  ll x = abs(arr[node][lr]-arr[child][0]) + dfs(child,node,0ll,dp);
  ll y = abs(arr[node ][lr]-arr[child][1]) + dfs(child,node,1ll,dp);
 
  sum+=max(x,y);
 }
 return dp[node][lr]=sum;
}
void solve()
{
 ll n;
 ...
 ll x = dfs(1ll,-1ll,0ll,dp);
 ll y = dfs(1ll,-1ll,1ll,dp);
 cout<<max(x,y)<<endl;
}
